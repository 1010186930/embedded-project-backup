...
u8 FTL_CreateLUT(u8 mode)
{
    ...
    for(i=0;i<nand_dev.block_totalnum;i++)
    {
		...
		if(buf[0]==0XFF&&mode)NAND_ReadSpare(i*nand_dev.block_pagenum+1,0,buf,1);//好块,且需要检查2次坏块标记。一个block只要看两个page就行了
		if(buf[0]==0XFF)//是好块 				 
        { 
    ...
    for(i=0;i<nand_dev.block_totalnum;i++)
    {
        if(nand_dev.lut[i]>=nand_dev.block_totalnum)//读到0xffff了
        {
...
u8 FTL_Format(void)
{
    ...
    goodblock=(nand_dev.good_blocknum*93)/100;	//%93的好块用于存储数据  ，标记了3809个
    n=0;										
