...
#define TRUE				1
#define FALSE				0
#define MAX_CWORD_LEN			18				//×î³¤µÄ´Ê
#define MAX_SWORD_LEN			256				//×î³¤µÄ¾ä×Ó
#define MAX_SPY_LEN			60                  //×î³¤µÄÆ´Òô
#define	MAX_CDIM			90                  //ºº×ÖÊ××Ö½ÚµÄ×î´ó±àÂëÓë×îĞ¡±àÂëµÄ²îÖµ£¬ÓÃÓÚÅĞ¶ÏÊÇ·ñÔÚºº×Ö±àÂë·¶Î§
#define	MAX_WORD			10000               //ÓÃÓÚÔÚ10000¸ö×Ö½ÚÊıÖĞÕÒ»»ĞĞ·û
#define	MAX_DATA            48000               //Ò»¶ÎÖĞ×î´óµÄÃ¤ÎÄ×Ö½ÚÊı
...
//´ÊµäË÷ÒıÊ±,×Ö»ò´Ê²»ĞèÒªË÷Òı
char *arrayChineseStop[] = {
"µÄ","Âğ","Ã´","°¡","Ëµ","¶Ô","ÔÚ","ºÍ","ÊÇ",
"±»","×î","Ëù","ÄÇ","Õâ","ÓĞ","½«","»á","Óë",
"ì¶","ÓÚ","Ëû","Ëı","Ëü","Äú","Îª","»¶Ó­"
};
typedef struct h2p
{
	char* py;
...
//±êµã·ûºÅ¼°ºº×ÖµÄ±êµã·ûºÅ,×¢Òâ + - " ÕâÈı¸ö·ûºÅ£¬ÒòÎªÔÚËÑË÷µÄÊ±ºòĞèÒªÍ¨¹ıËûÃÇ½øĞĞÒì»òµÈÌõ¼şÅĞ¶Ï
...
typedef struct _WORD_NODE
{
	...
	// todo ,¿ÉÒÔÔö¼Ó Á½¸ö×Ö£¬Èı¸ö×Ö£¬ËÄ¸ö×Ö£¬Îå¸ö×ÖµÄÊı×é£¬ÕâÑù²éÆğÀ´¸ü¿ì
	struct  _WORD_NODE	*nextWord;
	}WORD_NODE;    //¶¨Òå½á¹¹ÌåÀàĞÍ

struct _CH_DICT {
	WORD_NODE	*lstWord;
}CH_DICT[MAX_CDIM][MAX_CDIM];    //¶¨Òå½á¹¹Ìå¶şÎ¬Êı×é±äÁ¿£º´Êµä


char strText[MAX_WORD];  //¶¨ÒåÊı×é´æ·Å´ÓÎÄ¼şÖĞ¶ÁÈëµÄ×Ö·û´®
char SEG_LIST[3/2*MAX_WORD];  //¶¨ÒåÊı×é´æ·Å·Ö´ÊºóµÄ¾ä´®
char strPy[MAX_SPY_LEN];
int WNUM_IN = 0;
int WNUM_OUT = 0;
...
char  *strTrim(char str[]) //¶Ô¶ÁÈë×Ö·û´®µÄÔ¤´¦Àí£¬Èç¹ûÊ××Ö·ûÊÇ¿Õ¸ñ¡¢»Ø³µ»ò»»ĞĞ·ûµÈ£¬È¥³ı´ı´¦Àí×Ö·û´®ÖĞµÄ¿Õ¸ñ¡¢»»ĞĞµÈ
{
  int firstchar=0;
  int endpos=0;
  int i;
  ...
  for(i=firstpos;i<=endpos;i++)
    str[i-firstpos]=str[i];    //È¥³ı´ı´¦Àí×Ö·û´®ÖĞµÄ¿Õ¸ñ¡¢»»ĞĞµÈ
  str[i-firstpos]='\0';

  return str;
}

int addDictWord(char *strWord , int len) //´æ´¢·Ö´Ê¹ı³ÌÖĞµÚÒ»¸ö×Ö½ÚºÍ×îºóÒ»¸ö×Ö½ÚÏàÍ¬µÄ´Ê£¬ÊÇ¶¯Ì¬Êı×é¡£
{
	...
	/*printf("newWord:%p,sizeof newWord:%d\n", newWord,sizeof(newWord));*/

	/*curTmp = (WORD_NODE*)malloc(sizeof(WORD_NODE));
	curLst = (WORD_NODE*)malloc(sizeof(WORD_NODE));*/
	if (firstChar < 161 || lastChar < 161 )	//½¨Á¢ºº×Ö´Êµä£¬²»ÔÊĞí³öÏÖ·Çºº×Ö»òÊÇºº×ÖµÄÈ«½Ç·ûºÅ
		  return -1;
	...
	firstChar -= 161 ;
	lastChar  -= 161 ;      //°´´Ê´®µÚÒ»¸ö×Ö½ÚºÍ×îºóÒ»¸ö×Ö½ÚË³ĞòÀ´´æ´Ê
	...
	curTmp = curLst ;    //curLst¶ÔÓ¦Î»ÖÃÉÏÒÑ¾­´æ´ÊÔòË³×ÅÖ¸ÕëÏòºóÕÒ
...
void freeDict() //Çå¿ÕaddDictWord£¨£©µÄCH_DICT[i][j].lstWord Á´±í
{
	int i ,j ;
...
void addSegWord( unsigned char *strWord, int len) //¼ÆËã·Ö´ÊµÃµ½´ÊÓïµÄ×Ö½ÚÊı
{
...
int freeSeg() //°Ñ\0ÌíÈëbtmpµÈÖĞ£¬ÓÃÓÚbtmpµÈÇåÁã
{
...
BOOL searchWord( unsigned char *strWord,int len ) //´ÊÓïÆ¥Åä
{
	...
	curLst = CH_DICT[firstChar][lastChar].lstWord;
	curTmp = curLst;
	if(!strcmp("üŠ¹Ä",(char*)strWord)||!strcmp("ÈıüŠ",(char*)strWord)||!strcmp("üŠ¹Ä°Õ",(char*)strWord)) curTmp=NULL; //¸ö±ğÉúÆ§×Öµ¼ÖÂcurTmp³ö´í£¬±ÈÈç¡°üŠ¡±×Ö£¬Òò´Ë×öÕâÑùµÄ´¦Àí
	while ( curTmp != NULL ) {			
...
int segWord ( unsigned char *strText  , int iWordLen , BOOL bChinese )//ÄæÏòÆ¥ÅäËã·¨
{
	...
	BOOL	bFound = FALSE;	
	i = iWordLen  ; 
	if ( FALSE == bChinese )//·Çºº×Ö´®´¦Àí£¬Ó¢ÎÄ»òÊı×Ö
	{			
		//¼ì²é ÊÇ·ñÔÚstopÊı×éÀï
			addSegWord(strText,iWordLen);
		//if(!strcmp((char*)strText,""))
		//{
		//	result=ftb((char*)strText);//Ìí¼Ó»»ĞĞ·û
		//	strcat(btmp,result); 
		//	memset(result,'\0',sizeof(result));
		//}
		//else	
  ...
  //  		strncat(btmp, "000000", 6);
	...
	while ( i > 1 ) 
	{		//i±íÊ¾È¡³öµÄ¾ä×ÓÖĞ»¹Î´Æ¥Åä³öÀ´µÄ×ÖÊı
		for ( j = MAX_CWORD_LEN ; j >= 2 ; j -=2 )
		{					//×î´ó´Ê³¤¸öºº×Ö
			if ( i < j )     //¾ä×ÓµÄ³¤¶ÈĞ¡ÓÚ×î´ó´Ê³¤Ê±£¬½øÈëÏÂ´ÎÑ­»·¼õĞ¡j
				continue;   
			l = 0 ;
			Lft=strlen((char *)strText)-j-temp;//¼õÈ¥Æ¥Åäµ½µÄ´Ê

/*			for ( k=temp , l=0 ; k<temp+j ; k++, l++)
			strChar[l] = strText[k];
			strChar[l] = '\0';       */

			for ( k=Lft , l=0 ; k<Lft+j ; k++, l++)  //ÄæÏò×î´ó
			strChar[l] = strText[k];
			strChar[l] = '\0'; 
			if ( 2 == j )  {	//µ¥¸öºº×Ö
...
//	strncat(btmp, "000000", 6);//	printf("%s\n",strChar);
			...
			bFound = searchWord(strChar , l );  //Æ¥Åä´Êµä
			if ( TRUE == bFound )
			{		
				i -= j;
				temp+=j;
...
//              CH_br((char*)strChar); 
			...
			/* memset(strPy,'\0',sizeof(strPy));*/
				...
				//1.ÊÇ·ñÈ«ÊÇÍâÀ´´Ê
				//2.ÊÇ·ñÈ«ÊÇÊı×Ö
				//3.Èç¹ûµÚÒ»¶ş×ÖÊÇ¡±ĞÕÊÏ¡°£¬ÔòÄ¬ÈÏÎªĞÕÃû
				//4.ÅĞ¶ÏÆäËüÃû×Ö
			}
		} // end for j
	} // end while $i > 0 */
...
int loadDict(char *strFilename) //¶Ô·Ö´Ê¿â½øĞĞ°´ÕÕÊ××Ö½ÚºÍÄ©×Ö½Ú´æ´¢
{
	int 	len = 0 , i ,j ;
	FILE 	*fpDict = NULL;
	unsigned char firstChar,lastChar;
    ...
    for ( i = 0 ; i < MAX_CDIM ; i ++ ) {
    	for ( j = 0 ; j < MAX_CDIM ; j ++ ) {
			CH_DICT[i][j].lstWord = (WORD_NODE*)malloc(sizeof(WORD_NODE));
    		CH_DICT[i][j].lstWord = (WORD_NODE *)NULL;
    	}
    }
...
{	
	...
	int         a=0;
	int			iNexti		= 0 ;
	BOOL		bSep	  	= FALSE ;//ÊÇ·ñÊÇ·Ö¸ô·û»òÕßÊÇÒ»¸ö´Ê½áÊø
	BOOL		bChinese	= FALSE ;//ÉÏÒ»¸öÓĞĞ§×Ö·ûÀàĞÍ£¬
						//false:Ó¢ÎÄ true: Chinese
	BOOL		bFound		= FALSE ;
	BOOL        bSymbol		= FALSE ;//ÊÇ·ñÊÇ±êµã
	unsigned 	char    strWord[MAX_SWORD_LEN + 1]; 		//µ±Ç°´Ê
	...
	strChar[0] = '\0';
		...
		if (128 > cChar) {					//Ó¢ÎÄ×Ö·û
			/****Á¬Ğø¿Õ¸ñ²»Ëã·Ö¸ô ****/
				if ( ' ' == cChar || '\t' == cChar || '\r' == cChar || '\n' == cChar ){
				if ( TRUE == bChinese ) {			//Èç¹ûÇ°ÃæÒ»¸öÓĞĞ§×Ö·ûÊÇChinese
					if ( TRUE == bSpace && (' ' == cChar || '\t' == cChar)) 
						bSep = TRUE ;			
					else continue ;				//¼ÌĞøÈ¡ÏÂÒ»¸ö×Ö·û£¬ÒòÎªºº×ÖµÄ´Ê¿ÉÒÔ»»ĞĞ»òÓÃ¿Õ¸ñ¸ô¿ª
				} else  {
					bSep = TRUE ;
				}
			} else { 						
				bSymbol = isAsciiSymbol(cChar);	//ÅĞ¶ÏÊÇ·ñÊÇÓ¢ÎÄ±êµã
				bSep = bSymbol;
			}     //È·¶¨bSep
			if ( (TRUE == bSep || TRUE == bChinese ) &&  iWordLen >= 0 ) {			//Ò»¸öµ¥´Ê½áÊø
				if ( iWordLen > MAX_SWORD_LEN )
					 iWordLen = MAX_SWORD_LEN;
				strWord[iWordLen] = '\0';    //ÕÒµ½Ò»¾ä¿ÉÒÔ¶Ï¿ªµÄ»°£¬¶Ô¸Ã¾ä»°½øĞĞ·Ö´Ê
		...
		/*result=ftb((char*)strWord);strcat(btmp,result);memset(result,'\0',sizeof(result));*/	
			...
			if ( FALSE == bSep ) {
				strWord[iWordLen] = cChar;
							//Í¬Ò»¸öµ¥´ÊµÄ×ÖÄ¸,²¢ÔÚÒ»Æğ
				iWordLen ++ ;
			}
			bChinese = FALSE ;     //È·¶¨bChinese
		} else {							//´¦Àí×Ö·ûChinese
			if ( FALSE == bChinese && FALSE == bSep && 0 < iWordLen ) {//ÒÔÇ°ÊÇÓ¢ÎÄ£¬Åöµ½ºº×Ö£¬¾Íµ±Ó¢ÎÄµ¥´Ê½áÊø
				if ( iWordLen > MAX_SWORD_LEN )
					 iWordLen = MAX_SWORD_LEN;
				strWord[iWordLen] = '\0';
				bFound = isEnglishStop(strWord);
				if ( FALSE == bFound ) { //¶ÔstrWord²ğ¿ª³Éµ¥¸ö×Ö½ÚÔÙ½øĞĞÃ¤ÎÄ×ª»»
		/*		    addSegWord(strWord,1);
			...
			iNexti	= i + 1 ;
			if ( iNexti <  iTextLen ) {
				if ( 128 > ((unsigned char)strText[iNexti]) ) {		//µ¥×Ö·û<=128£¬²»´¦Àí¸Ã×Ö·û
					continue ;					
				}
			} else {						//×îºóÒ»¸ö×Ö·ûÊÇ´óÓÚ128µÄµ¥×Ö·û
				break;
			}
			...
			strChar[2] = '\0';
						//ÊÇ·ñÓĞ¿ÉÄÜ>128µÄ×Ö·û£¬Ö»ÓĞÒ»¸ö×Ö·û£¬Èç¹ûÓĞĞèÒªÁíÍâÅĞ¶Ï£¬ÏÖÔÚÄ¬ÈÏÒ»¶¨»áÓĞÖÁÉÙÁ½¸ö×Ö·ûÍ¬Ê±³öÏÖ
			bChinese	= TRUE ;				//ÊÇºº×ÖChinese
			i ++ ;						//Ö»ĞèÒª¼Ó1
			
			if (strChar[0] == 0xa1 && strChar[1] == 0xa1 ) {				//Chinese¿Õ¸ñ,16½øÖÆ:A1,A1
				if ( TRUE == bSpace )
					bSep = TRUE ;
				else continue ;					//Á¬ĞøChinese¿Õ¸ñ
			}else if ( strChar[0] < 176 ) {//ÖĞÎÄ±êµãµÈ·Çºº×Ö×Ö·û
				bSep = TRUE ;
				bSymbol = TRUE ;
			}else bSep = FALSE;
...
int main()//int argc,char *argv[])
{
	...
	printf("ÔØÈë´Êµä\n");
	printf("sizeof dict before:%d\n", sizeof(CH_DICT));
	printf("sizeof dict newWord:%d\n", sizeof(WORD_NODE));
	loadDict("d:\\output\\ziguang.txt");
//	loadDict("d:\\output\\ziguang.txt","d:\\output\\Æ´Òô.txt");
	initSegList();
	...
	/*while (1) {*/
		...
		/*strcpy(strText, "ÄãºÃÎÒ½ĞÁÖ¶°");
		segSentence(strText, FALSE);*/
	GBtoSegwithDot("°¢²¼ÀÊ°×ÖçµØ°×Öó");
		/*fwrite(SEG_LIST, WNUM_OUT, 1, fpfcOut);+		[0x00000010]	0x00712fe0 {{lstWord=0x00b2ec18 {strWord=0x00b2ec18 "°¢±È¸Ç¶û" strPY=0x00b2ec2b "ÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ... ...} }, ...}	_CH_DICT[0x0000005a]
		...
		fputc('\n', fpdataOut);*/
		printf("btmp:%s\n", btmp);
		printf("sizeof dict:%d\n", sizeof(CH_DICT));
		printf("sizeof p2h:%d\n", sizeof(p2h));
		printf("sizeof arrayEnglishStop:%d\n", sizeof(arrayEnglishStop));
		printf("sizeof arrayChineseStop:%d\n", sizeof(arrayChineseStop));
		printf("sizeof np:%d\n", sizeof(np));
		printf("sizeof arrayAsciiSymbol:%d\n", sizeof(arrayAsciiSymbol));
		printf("sizeof p2b_danfang:%d\n", sizeof(p2b_danfang));
		printf("sizeof p2b_ym:%d\n", sizeof(p2b_ym));
		printf("sizeof p2b_sm:%d\n", sizeof(p2b_sm));
