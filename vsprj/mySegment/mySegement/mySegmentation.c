...
char segpath[] = ".\\Debug\\FenciTable.txt";
FILE* pSeg = NULL;
...
int StringMatch(unsigned short* src, int length)
{
	/*FILE* pSeg = NULL;*/
	unsigned short* psrc = src;
	/*unsigned short unitmp[10] = { 0x00 };*/
	unsigned short* unitmp = (unsigned short*)malloc(sizeof(unsigned short) * length);
	...
	while (low <= high)//dichotomous method
	{
		...
		fseek(pSeg, offset, 0);
		fread(unitmp, sizeof(unsigned short), length, pSeg);
...
int StringSegment(unsigned short* dst, unsigned short* src, int src_len,int* seg_dst_len)
{
	unsigned short* pdst = dst;
	unsigned short* psrc = src;
	int get_len = 0;
...
int DevideString(unsigned short* dst, unsigned short* src, int len)
{
	unsigned short* pdst = dst;
	unsigned short* psrc = src;
	int isChange = 0;//means the current character is not a Chinese character
