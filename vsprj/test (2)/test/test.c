...
void newalign_120to37();
int main()
{
	//char buffer2[100] = "";
	//char buff_Totranslate[21] = "";
	//char buffer[] = "adcdshkhshakdhasddhgg";
	...
	//	else
	//		strcpy(buff_Totranslate, buffer + 1);//将包括空格的内容保存到buff_Totranslate
	//}
	//else
	//	strcpy(buff_Totranslate, buffer + 1);
	//unsigned int a = ((unsigned int)(2015 - 1980) << 25) /* Year 2015 */
	//	 | ((unsigned int)1 << 21) /* Month 1 */
	//	 | ((unsigned int)1 << 16) /* Mday 1 */
	//	 | ((unsigned int)0 << 11) /* Hour 0 */
	//	 | ((unsigned int)0 << 5) /* Min 0 */
	//	 | ((unsigned int)0 >> 1); /* Sec 0 */
	//printf("%x\n", a);
	//printf("》时间戳: %u/%02u/%02u, %02u:%02u\n",
	//	(a >> 9) + 1980, a >> 5 & 15, a & 31, a >> 11, a >> 5 & 63);

	/*const char a[] = "üān  üán   üǎn   üàn";
	const char srcSubStr[] = "ü";
	const char dstSubStr[] = "v";
	char out[150]= "";
	ReplaceSubStr(a, srcSubStr, dstSubStr, out);
	printf("%s", out);*/
	align_120to37();
	/*else if (buffer[19] == ' ' && buffer[20] != ' ')
		strcpy(buff_Totranslate, buffer);*/
	/*printf("%s", buff_Totranslate);*/
	//char str[10][100] = { 
...
void align_120to37(void)
{
	...
	FILE* fd1 = NULL;
	FILE* fd2 = NULL;
	fopen_s(&ft1, "C:\\Users\\10101\\Desktop\\分词结果.txt", "rb");//分词结果用ft1表示
	...
	while (1)
	{
		...
		printf("buffert:%s", buffert);
		/*for (int flag = 0; flag < 126; flag++)
		{
			if (feof(fd1))
				break;
			else
...
void newalign_120to37()
{
	...
	char kong[7] = "000000";
	int l, n, i, j, k, s = 0;
	int blankNumsInData = 0, blankNumsInText = 0, t = 0,cnt = 0;
	int m1 = 0, m2 = 0, m = 0;
	while (1)
	{
		fseek(ft1, t, SEEK_SET);//从ft1的t位置开始读
		...
		else//大于120即126，构成了20方和21方
		{
			...
			for (k = 0; k < 6; k++)//拿出第21方到br2
				br2[k] = bufferd[6 * 20 + k];
			br2[6] = '\0';
			int temp = strcmp(br1, kong);
			int temp2 = strcmp(br2, kong);
			if ((strcmp(br1,kong))&&(strcmp(br2, kong)))//br1、br2都不为空方
			{
				//计算多少个空格
				for (j = 18, blankNumsInData = 0; j >= 0; j--)//遍历盲文块的前19格
				{
					for (n = 0; n < 6; n++)
						br0[n] = bufferd[6 * j + n];
					br0[6] = '\0';//每6个存入字符数组br0，最后一位添置结束符
					if (!strcmp(br0,kong))//如果6个点都是0，则代表空方
						blankNumsInData++;//前19格盲文块有j1个空方
				}
				printf("第20和21方都不是空方，前19方的空方数%d\n", blankNumsInData);//第一次printf输出前19格的空方数

				//处理分词结果文本
				for (j = 0, blankNumsInText = 0; j < 37; j++)//循环37次
				{
					if (buffert[j] == ' ')//如果buffert[j]是空格
						blankNumsInText++;//j2计数
					fputc(buffert[j], ft2);//把buffert[j]写入ft2
					if (blankNumsInData == blankNumsInText) 
						break;//当分词结果的前37个字符的空格数等于前19格盲文块的空格数时，跳出循环
				}
				for (cnt = 0; cnt < 36 - j; cnt++)//计算要补多少个空格ft2才能满37个字符
					fputc(' ', ft2);//补空格
				t = t + j + 1;//偏移量为上次读到位置的后面一位，由于上面的for循环是调用的全局变量j，所以值可以传下去！这点和在for循环里面int j不同！
				for (j = 18; j >= 0; j--)//遍历盲文块的前19格，1-19个
				{
					for (n = 0; n < 6; n++)
						br0[n] = bufferd[6 * j + n];
					br0[6] = '\0';
					if (!strcmp(br0, kong))//倒着判断，如果遇到空方
						break;//跳出该循环
				}
				for (k = 0; k <= j; k++)
					for (n = 0; n < 6; n++)
			...
			else
			{
				for (k = 0; k < 120; k++)
					fputc(bufferd[k], fd2);//先读取前120个点到fd2
				for (j = 19, blankNumsInData = 0; j >= 0; j--)//取前20方
				{
					for (n = 0; n < 6; n++)
						br0[n] = bufferd[6 * j + n];
					br0[6] = '\0';
					if (!strcmp(br0, kong))
						blankNumsInData++;//判断前20方有几个空格
				}
				if ((!strcmp(br1, kong)) && (strcmp(br2, kong)))//如果第20方是空格第21方不是空方
				{
					for (j = 0, blankNumsInText = 0; j < 37; j++)//判断前37个字符有多少个空格
					{
						if (buffert[j] == ' ')
							blankNumsInText++;
						fputc(buffert[j], ft2);
						if (blankNumsInText == blankNumsInData)
							break;//两者空格数一样后跳出该循环
					}
					printf("第20是空方第21方不是空方，前20方的空方数%d\n", blankNumsInData);
				}
				else if((strcmp(br1, kong)) && !(strcmp(br2, kong)))//如果第20方不是空格，分词结果的空格就要多数一格，不然会漏掉最后的一个字
				{
					for (j = 0, blankNumsInText = 0; j < 37; j++)
					{
						if (buffert[j] == ' ')
							blankNumsInText++;
						if (blankNumsInText == blankNumsInData + 1)
						{
							j = j - 1;
							break;
						}
					...
					printf("第20不是空方第21方是空方，前20方的空方数%d\n", blankNumsInData);
				}
				else if ((strcmp(br1, kong)) && (strcmp(br2, kong)))
				{
					for (j = 0, blankNumsInText = 0; j < 37; j++)//判断前37个字符有多少个空格
					{
						if (buffert[j] == ' ')
					...
					printf("第20是空方第21方是空方，前20方的空方数%d\n", blankNumsInData);
				}
				for (cnt = 0; cnt < 36 - j; cnt++)//补满37个
					fputc(' ', ft2);
				t = t + j + 1;//更新偏移量
				s = s + 120;//更新偏移量
			}
